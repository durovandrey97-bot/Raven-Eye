...
(function(){
  ...
  function nextStep(){
    ...
    if(txt) txt.textContent = steps[step][1];
    step++;
  }
  nextStep();
  var iv = setInterval(nextStep, 600);
  window._splashInterval = iv;
})();
...
// Принудительно скрываем через 5 секунд если Firebase не ответил
...
async function tgSendToUser(chatId,text){
  ...
  try{
    ...
    const result = await apiCall('sendTg', {chatId: chatId, text: text});
          ...
          if(t.fixed&&!notifiedFixed.has(t._id)){
            ...
            if(currentUser?.tgChatId){
              ...
              tgSendToUser(currentUser.tgChatId,
                '💰 <b>Фиксируйте прибыль!</b>\n' +
                '━━━━━━━━━━━━━━\n\n' +
                fixChainEmoji+' <b>'+t.pair+'</b>\n' +
                '📍 '+(CHAIN_META[t.chain]?.label||t.chain)+'\n\n' +
                '⚡ <b>Время выходить из сделки!</b>\n\n' +
                '🔗 <a href="'+SITE_URL+'">Открыть RavenEye</a>'
              );
            }
          }
          knownIds.add(t._id);
