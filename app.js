  ...
  try{
    ...
    try{
      var _loginEntry = {
        ...
        browser: /Safari/.test(navigator.userAgent)&&!/Chrome/.test(navigator.userAgent)?'Safari':/Chrome/.test(navigator.userAgent)?'Chrome':'Other',
      ...
      // Пишем в подколлекцию login_history (последние 10)
      db.collection('users').doc(raw).collection('login_history').add(_loginEntry).catch(function(){});
    }catch(e){}
