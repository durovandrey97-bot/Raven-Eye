  ...
  try{
    tradesUnsub=db.collection('trades').onSnapshot(
      async snap=>{
        ...
        const allTrades=[];
        snap.forEach(d=>{
          const td={...d.data(),_id:d.id};
          // Нормализуем createdAt → createdAtMs для надёжного isExpired
          if(!td.createdAtMs){
            if(td.createdAt && td.createdAt.toDate) td.createdAtMs = td.createdAt.toDate().getTime();
            else if(td.createdAt && td.createdAt.seconds) td.createdAtMs = td.createdAt.seconds*1000;
            else if(typeof td.createdAt==='string') td.createdAtMs = new Date(td.createdAt).getTime();
          }
          allTrades.push(td);
        });
        // Фильтр по userId/userIds: показываем только свои или общие (без пользователей)
        const newTrades=allTrades.filter(function(t){
          // Общая сделка
          if(!t.userId && (!t.userIds||!t.userIds.length)) return true;
          // Новый формат userIds
          if(t.userIds && t.userIds.indexOf(currentUser.id) !== -1) return true;
          // Старый формат userId
...
function autoFixExpiredTrades(tradesList){
  ...
  tradesList.forEach(function(t){
    ...
    if(!d) return;                               // нет времени создания — пропускаем
    var tmaxMs = (t.tmax||30) * 60 * 1000; // фиксируем в момент tmax
...
window.doLogout=function(){
  removeUserId();
  // Очищаем все данные текущего пользователя из localStorage
  var keysToRemove = [
    'dex_seen_ids_v2',
    'dex_notified_fixed_v2',
    'dex_balance_history',
    'dex_offline_queue',
    '_lastBalChartKey'
  ];
  // Также удаляем ключи привязанные к userId
  if(currentUser && currentUser.id){
    keysToRemove.push('raven_profit_history_'+currentUser.id);
    keysToRemove.push('dex_onboarding_'+currentUser.id);
  }
