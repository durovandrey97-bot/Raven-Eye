...
async function tgSendToUser(chatId,text){
  ...
  var now=Date.now();
  if(_tgSentCache[cacheKey]&&now-_tgSentCache[cacheKey]<30000){
...
function getUserId() {
  return localStorage.getItem('dex_user_id') || getCookie('dex_user_id') || null;
}
function saveUserId(id) {
  localStorage.setItem('dex_user_id', id);
  setCookie('dex_user_id', id, 365); // 1 год
}
