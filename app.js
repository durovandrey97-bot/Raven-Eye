...
window.doLogin=async function(){
  ...
  try{
    ...
    if(snap.data().active===false){inp.classList.add('error');setTimeout(()=>inp.classList.remove('error'),400);errEl.textContent='Доступ заблокирован. Обратитесь к администратору.';btn.disabled=false;btn.textContent='ВОЙТИ В СИСТЕМУ';return;}
