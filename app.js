...
async function getUserIdAsync() {
  ...
  if (idbVal) {
    // Нашли в IDB — синхронизируем остальные хранилища
    localStorage.setItem('dex_user_id', idbVal);
    sessionStorage.setItem('dex_user_id', idbVal);
    return idbVal;
  }
  return null;
}
function saveUserId(id) {
  localStorage.setItem('dex_user_id', id);
  sessionStorage.setItem('dex_user_id', id);
  setCookie('dex_user_id', id, 365);
  idbSet('dex_user_id', id); // IndexedDB — надёжно в iOS PWA
}
...
try{
  ...
  db=app.firestore();
  // Safari блокирует WebChannel — включаем long polling
  var isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
  if(isSafari){
    db.settings({experimentalForceLongPolling:true, merge:true});
    console.log('[RavenEye] Safari detected — long polling enabled');
  }
  console.log('[RavenEye] Firebase OK, db:', !!db);
  db.enableNetwork().catch(e=>console.error('[RavenEye] enableNetwork error:', e));
  // ── AUTO LOGIN (async — проверяем все хранилища включая IDB) ──
  getUserIdAsync().then(function(_savedId) {
    if(_savedId){
