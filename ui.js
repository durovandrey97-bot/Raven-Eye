...
function openFaq(){
  ...
  const faqs=[
    {q:'Что делать если пропустил сигнал?',a:'Ничего страшного. Каждый день приходит несколько новых сигналов по расписанию. Пропущенная сделка просто исчезнет из списка после истечения окна входа — просто жди следующую.'},
    {q:'Что значат цифры спреда, например 0.8% → 1.5%?',a:'Это диапазон арбитражной разницы между ценами на разных пулах. 0.8% — минимальный спред при входе, 1.5% — максимальный. Чем выше правая цифра — тем потенциально выгоднее сделка.'},
    ...
    {q:'Можно ли брать несколько сделок одновременно?',a:'В интерфейсе одновременно активна одна сделка с таймером. Но ты можешь видеть все сигналы и выбирать наиболее выгодный перед тем как нажать "Взять".'},
  ];
