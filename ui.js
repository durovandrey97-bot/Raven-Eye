...
function updateTopbarChart(){
  ...
  var balEl  = document.getElementById('topbar-balance');
  ...
  // Баланс
  if(balEl) balEl.textContent = fmtBal(balance);

  // Данные для графика — берём историю прибыли
  var history = [];
  try{
  ...
  // Если меньше 2 точек — просто линия
  var points = history.slice(-12).map(function(e){ return e.balance||0; });
  if(points.length < 2) points = [balance * 0.9, balance];
  ...
  // Цвет баланса
  if(balEl) balEl.style.color = color;
}
