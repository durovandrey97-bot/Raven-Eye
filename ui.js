...
async function render(){
  ...
  const borderW=expired?'3px':Math.max(3,Math.min(8,Math.round((t.hi||0)*2.5)))+'px';
    ...
    const chainColorVar=sel?`--chain-color:${chainColor};`:'';
    const fixedColorVar=t.fixed?'--chain-color:#14b8a6;':'';
    return `<div class="trade-item-wrap" data-wrap-id="${t._id}">

      <div class="trade-item${sel?' selected':''}${expired?' expired':''}${locked?' locked':''}${t.fixed?' fixed-trade':''}" style="animation-delay:${i*.04}s;border-left:${borderW} solid ${t.fixed?'#14b8a6':borderColor};background-image:linear-gradient(135deg,${t.fixed?'rgba(20,184,166,.06)':chainGrad} 0%,transparent 55%);${chainColorVar}${fixedColorVar}" data-id="${t._id}">
      ${lockHtml}
      <div class="ti-top">
