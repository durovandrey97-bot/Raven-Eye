...
function showMainApp(){
  ...
  function updatePresence(){
    ...
    try{
      db.collection('presence').doc(currentUser.id).set({
        online:true,
  ...
  updatePresence();
  setInterval(updatePresence, 30000); // каждые 30 сек
  window.addEventListener('beforeunload',function(){
    try{db.collection('presence').doc(currentUser.id).set({online:false,lastSeen:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});}catch(e){}
  });
  // Возврат на страницу — обновляем присутствие
