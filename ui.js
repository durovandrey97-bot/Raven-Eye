...
function getAddedTime(t){
  try{
    if(t.createdAtMs && !isNaN(t.createdAtMs)) return new Date(t.createdAtMs);
    if(!t.createdAt) return null;
    if(t.createdAt.toDate){ const d=t.createdAt.toDate(); if(d&&!isNaN(d.getTime())) return d; }
    if(t.createdAt.seconds){ return new Date(t.createdAt.seconds*1000); }
    if(typeof t.createdAt==='string'){ const d=new Date(t.createdAt); if(!isNaN(d.getTime())) return d; }
    return null;
  }catch(e){ return null; }
}
function hasRealTime(t){return !!(t.createdAtMs||t.createdAt?.seconds||t.createdAt?.toDate||(typeof t.createdAt==='string'&&t.createdAt.length>8));}
function isExpired(t){
  const d=getAddedTime(t);
  if(!d){
    if(t.fixed) return true; // fixed без времени — скрываем
    return false;
  }
  const elapsed = Date.now()-d.getTime();
  if(isNaN(elapsed)) return false; // некорректная дата — не скрываем
  const windowMs=((t.tmax||30)+10)*60*1000;
...
function getFiltered(){
  let list=trades.filter(t=>{
    if(isExpired(t)) return false;         // скрываем истёкшие
...
window.setChain=function(c,btn){
  ...
  if(c==='history'){chain='history';renderHistory();updateStats(trades);}
  else{
    chain=c;
...
async function render(){
        ...
        <div>
          <div class="pair-row">
            ...
            <button class="share-btn" data-share-id="${t._id}" title="Поделиться">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="pointer-events:none"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
            </button>
            <button class="copy-btn" data-copy="${t.pair||''}" title="Скопировать">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="pointer-events:none"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
            </button>
          </div>
  ...
  if(list.length > 8){
    ...
    if(col2){
      if(col2._vsObserver){ col2._vsObserver.disconnect(); col2._vsObserver=null; }
      col2._vsObserver = new IntersectionObserver(function(entries){
        entries.forEach(function(e){
          var inner = e.target.querySelector('.trade-item');
...
function loadAlerts(){
  // Очищаем старые ключи alerts (старше 3 дней)
  try{
    var cutoff = Date.now() - 3*24*60*60*1000;
    Object.keys(localStorage).forEach(function(k){
      if(k.startsWith('dex_alerts_')){
        var dateStr = k.replace('dex_alerts_','');
        if(new Date(dateStr).getTime() < cutoff) localStorage.removeItem(k);
      }
    });
  }catch(e){}
  try{const raw=localStorage.getItem('dex_alerts_'+new Date().toISOString().slice(0,10));if(!raw)return;const{date,alerts:saved}=JSON.parse(raw);if(date===new Date().toISOString().slice(0,10)&&saved.length)alerts=saved;}catch(e){}
}
function addAlert(t){
...
function openGmailModal(){
  var old=document.getElementById('gmail-modal');
  if(old)old.remove();
  var savedEmail = '';
  try{ savedEmail = localStorage.getItem('raven_report_email')||''; }catch(e){}

  var modal=document.createElement('div');
  modal.id='gmail-modal';
  modal.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.75);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px;animation:fadeIn .2s ease';
  modal.innerHTML=`
    <div style="background:#0d1017;border:1px solid rgba(255,255,255,.1);border-radius:20px;padding:24px 20px;width:100%;max-width:380px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
        <div style="width:40px;height:40px;border-radius:12px;background:rgba(234,67,53,.12);border:1px solid rgba(234,67,53,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2z" stroke="#EA4335" stroke-width="1.8"/><polyline points="22,6 12,13 2,6" stroke="#EA4335" stroke-width="1.8"/></svg>
        </div>
        <div>
          <div style="font-size:15px;font-weight:700;color:#fff">Отчёты на email</div>
          <div style="font-size:11px;color:rgba(255,255,255,.4);margin-top:2px">Ежемесячная статистика прибыльности</div>
        </div>
        <button onclick="document.getElementById('gmail-modal').remove()" style="margin-left:auto;width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.5);cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center">✕</button>
      </div>
      ...
      <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:14px;margin-bottom:16px">
        <div style="font-size:12px;color:rgba(255,255,255,.5);line-height:1.7">
          📊 Каждый месяц вам будет приходить детальный отчёт по торговле:<br>
          <span style="color:rgba(255,255,255,.35)">• Количество сделок и процент прибыльных<br>
          • Средний доход за сделку<br>
          • Динамика капитала по неделям<br>
          • Лучшие пары и сети за месяц</span>
        </div>
      </div>

      <div style="margin-bottom:14px">
        <label style="font-size:11px;font-weight:600;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.05em;display:block;margin-bottom:7px">Ваш Gmail</label>
        <input id="gmail-input" type="email" placeholder="example@gmail.com" value="${savedEmail}"
          style="width:100%;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:12px 14px;font-size:14px;color:#fff;outline:none;font-family:inherit;transition:border .2s"
          onfocus="this.style.borderColor='rgba(234,67,53,.5)'" onblur="this.style.borderColor='rgba(255,255,255,.12)'"/>
      </div>

      <button onclick="saveGmail()" style="width:100%;padding:13px;border-radius:12px;background:linear-gradient(135deg,rgba(234,67,53,.25),rgba(234,67,53,.12));border:1px solid rgba(234,67,53,.4);color:#EA4335;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:8px">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2z" stroke="#EA4335" stroke-width="1.8"/><polyline points="22,6 12,13 2,6" stroke="#EA4335" stroke-width="1.8"/></svg>
        Сохранить и подписаться
      </button>
      ${savedEmail ? `<button onclick="removeGmail()" style="width:100%;margin-top:8px;padding:10px;border-radius:12px;background:transparent;border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.3);font-size:12px;cursor:pointer;font-family:inherit">Отписаться</button>` : ''}
    </div>`;

  modal.addEventListener('click',function(e){if(e.target===modal)modal.remove();});
  document.body.appendChild(modal);
  setTimeout(function(){ document.getElementById('gmail-input')?.focus(); }, 100);
}

window.saveGmail = function(){
  var email = (document.getElementById('gmail-input')?.value||'').trim();
  if(!email || !email.includes('@')){
    showToast('Введите корректный email','error');
    return;
  }
  try{ localStorage.setItem('raven_report_email', email); }catch(e){}
  // Сохраняем в Firebase
  if(currentUser && db){
    db.collection('users').doc(currentUser.id).update({reportEmail: email}).catch(function(){});
    apiCall('saveUser',{userId:currentUser.id,reportEmail:email}).catch(function(){});
  }
  document.getElementById('gmail-modal')?.remove();
  // Обновляем иконку если email задан
  var btn = document.getElementById('gmail-btn');
  if(btn) btn.style.color = '#EA4335';
  showToast('✉️ Email сохранён — отчёты будут приходить ежемесячно','success');
};

window.removeGmail = function(){
  try{ localStorage.removeItem('raven_report_email'); }catch(e){}
  if(currentUser && db){
    db.collection('users').doc(currentUser.id).update({reportEmail: null}).catch(function(){});
  }
  document.getElementById('gmail-modal')?.remove();
  var btn = document.getElementById('gmail-btn');
  if(btn) btn.style.color = '';
  showToast('Отписались от отчётов','success');
};

function openFaq(){
  var old=document.getElementById('faq-modal');
  if(old)old.remove();
  var modal=document.createElement('div');
  modal.id='faq-modal';
  modal.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.85);backdrop-filter:blur(12px);display:flex;align-items:flex-end;justify-content:center;animation:fadeIn .2s ease';

  const faqs=[
    {q:'Как часто приходят сигналы?',a:'Обычно 5–7 сигналов в день в зависимости от вашего капитала и доступных сетей. Расписание формируется автоматически каждый день с утра до вечера.'},
    {q:'Что значат цифры спреда, например 0.8% → 1.5%?',a:'Спред — это разница в цене одного актива между двумя DEX пулами. 0.8% — минимальная разница при входе в сделку, 1.5% — максимальная которую можно зафиксировать при выходе. Это и есть твой потенциальный доход от суммы сделки. Чем выше правая цифра — тем выгоднее сделка.'},
    {q:'Почему сделка исчезла?',a:'Сделка автоматически скрывается когда истекает окно входа (например 2–10 мин). Это нормально — арбитражное окно закрылось, рынок выровнялся. Жди следующий сигнал.'},
    {q:'Почему не пришло TG уведомление?',a:'Проверь: 1) Telegram подключён в профиле (должна быть галочка). 2) Уведомления от бота не отключены в Telegram. 3) Если включён фильтр по сетям — сигналы в недоступных сетях не приходят.'},
    {q:'Что делать если случайно закрыл приложение во время сделки?',a:'Откройте приложение снова — активная сделка восстановится автоматически, таймер продолжит отсчёт. Главное успеть зафиксировать прибыль до истечения окна входа.'},
    {q:'Безопасно ли хранить свой ID?',a:'ID используется только для входа в RavenEye. Он не даёт доступа к вашему кошельку или средствам — RavenEye является исключительно инструментом для сигналов и никак не подключается к вашему кошельку. Но всё равно не передавайте ID третьим лицам.'},
    {q:'Хранит ли RavenEye мои деньги?',a:'Нет. RavenEye — это инструмент который даёт торговые сигналы. Приложение не хранит, не принимает и не имеет доступа к вашим средствам. Оно не может подключиться к вашему кошельку. Все сделки вы совершаете исключительно внутри своего кошелька на DEX биржах — самостоятельно и в полной безопасности.'},
    {q:'Что такое "Взять сделку"?',a:'Это фиксация намерения войти в позицию. Нажав кнопку ты запускаешь таймер — в это время нужно реально провести своп на DEX бирже. Когда истечёт минимальное время окна — нажимаешь "Зафиксировать прибыль".'},
  ];

  modal.innerHTML=`
    <div style="width:100%;max-width:480px;max-height:93vh;overflow-y:auto;background:#0d1017;border-radius:24px 24px 0 0;padding:0 0 52px;scrollbar-width:none">
      <div style="position:sticky;top:0;z-index:10;background:#0d1017;padding:16px 20px 13px;border-bottom:1px solid rgba(255,255,255,.06);display:flex;align-items:center;justify-content:space-between">
        <div style="display:flex;align-items:center;gap:10px">
          <div style="width:36px;height:36px;border-radius:11px;background:rgba(41,182,246,.12);border:1px solid rgba(41,182,246,.25);display:flex;align-items:center;justify-content:center;font-size:18px">❓</div>
          <div>
            <div style="font-size:15px;font-weight:700;color:#29B6F6">Частые вопросы</div>
            <div style="font-size:10px;color:rgba(255,255,255,.3);font-family:'JetBrains Mono',monospace;letter-spacing:.05em">FAQ · УСТАНОВКА ПРИЛОЖЕНИЯ</div>
          </div>
        </div>
        <button onclick="document.getElementById('faq-modal').remove()" style="width:32px;height:32px;border-radius:50%;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.5);cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center">✕</button>
      </div>
      <div style="padding:20px 20px 0">

        <!-- Установка приложения -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">КАК УСТАНОВИТЬ ПРИЛОЖЕНИЕ</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:24px">
          <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:14px">
            ...
            <div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:8px">iPhone / iPad</div>
            ${['Открой Safari','Нажми кнопку Поделиться ↑','Выбери "На экран Домой"','Нажми "Добавить"'].map((s,i)=>`
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px">
              <div style="width:16px;height:16px;border-radius:50%;background:rgba(41,182,246,.2);color:#29B6F6;font-size:9px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0">${i+1}</div>
              <span style="font-size:11px;color:rgba(255,255,255,.5)">${s}</span>
            </div>`).join('')}
          </div>
          <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:14px">
            <div style="font-size:20px;margin-bottom:8px">🤖</div>
            <div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:8px">Android</div>
            ${['Открой Chrome','Нажми ⋮ в углу','Выбери "Установить приложение"','Нажми "Установить"'].map((s,i)=>`
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px">
              <div style="width:16px;height:16px;border-radius:50%;background:rgba(29,185,84,.2);color:#1db954;font-size:9px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0">${i+1}</div>
              <span style="font-size:11px;color:rgba(255,255,255,.5)">${s}</span>
            </div>`).join('')}
          </div>
        </div>
        <div style="background:rgba(212,175,55,.05);border:1px solid rgba(212,175,55,.15);border-radius:12px;padding:12px 14px;margin-bottom:24px">
          <div style="font-size:12px;color:rgba(255,255,255,.5);line-height:1.6">💡 После установки приложение работает как нативное — быстрее открывается, уведомления приходят надёжнее, не нужно каждый раз открывать браузер</div>
        </div>

        <!-- FAQ -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">ЧАСТЫЕ ВОПРОСЫ</div>
        <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:24px">
          ${faqs.map((f,i)=>`
          <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:13px;overflow:hidden">
            <button onclick="var b=this.parentNode.querySelector('.faq-ans');b.style.display=b.style.display==='none'?'block':'none';this.querySelector('.faq-arr').style.transform=b.style.display==='none'?'rotate(0deg)':'rotate(90deg)'" style="width:100%;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:13px 14px;background:none;border:none;cursor:pointer;font-family:inherit;text-align:left">
              <span style="font-size:13px;font-weight:600;color:#fff;line-height:1.4">${f.q}</span>
              <svg class="faq-arr" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.3)" stroke-width="2" style="flex-shrink:0;transition:transform .2s"><polyline points="9 18 15 12 9 6"/></svg>
            </button>
            <div class="faq-ans" style="display:none;padding:0 14px 13px;font-size:12px;color:rgba(255,255,255,.5);line-height:1.6;border-top:1px solid rgba(255,255,255,.05)">${f.a}</div>
          </div>`).join('')}
        </div>

        <!-- Поддержка -->
        <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:16px;text-align:center">
          <div style="font-size:13px;color:rgba(255,255,255,.5);margin-bottom:12px">Не нашли ответ на свой вопрос?</div>
          <a href="https://t.me/RavenEyeSupport" target="_blank" style="display:inline-flex;align-items:center;gap:8px;padding:11px 20px;border-radius:12px;background:rgba(41,182,246,.1);border:1px solid rgba(41,182,246,.25);color:#29B6F6;font-size:13px;font-weight:700;text-decoration:none">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="#29B6F6"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.248l-2.032 9.571c-.145.658-.537.818-1.084.508l-3-2.21-1.447 1.394c-.16.16-.295.295-.605.295l.213-3.053 5.56-5.023c.242-.213-.054-.333-.373-.12L8.32 13.617l-2.96-.924c-.642-.204-.657-.643.136-.953l11.527-4.443c.535-.194 1.003.131.59.168z"/></svg>
...
function openSecurity(){
  var old = document.getElementById('security-modal');
  if(old) old.remove();
  ...
  modal.innerHTML = `
    <div style="width:100%;max-width:480px;max-height:93vh;overflow-y:auto;background:#0d1017;border-radius:24px 24px 0 0;padding:0 0 52px;scrollbar-width:none">
      ...
      <div style="position:sticky;top:0;z-index:10;background:#0d1017;padding:16px 20px 13px;border-bottom:1px solid rgba(255,255,255,.06);display:flex;align-items:center;justify-content:space-between">
        <div style="display:flex;align-items:center;gap:10px">
          <div style="width:36px;height:36px;border-radius:11px;background:rgba(29,185,84,.12);border:1px solid rgba(29,185,84,.25);display:flex;align-items:center;justify-content:center;font-size:18px">🛡️</div>
          <div>
            <div style="font-size:15px;font-weight:700;color:#1db954">Безопасность</div>
            <div style="font-size:10px;color:rgba(255,255,255,.3);font-family:'JetBrains Mono',monospace;letter-spacing:.05em">КАК БЕЗОПАСНО РАБОТАТЬ С DEX</div>
          </div>
        </div>
        <button onclick="document.getElementById('security-modal').remove()" style="width:32px;height:32px;border-radius:50%;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.5);cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center">✕</button>
      </div>

      <div style="padding:20px 20px 0">

        <!-- Вводный баннер -->
        <div style="background:linear-gradient(135deg,rgba(29,185,84,.12),rgba(29,185,84,.03));border:1px solid rgba(29,185,84,.25);border-radius:18px;padding:18px 20px;margin-bottom:24px">
          <div style="font-size:14px;font-weight:700;color:#fff;margin-bottom:6px">DEX — это не страшно</div>
          <div style="font-size:13px;color:rgba(255,255,255,.5);line-height:1.7">
            Децентрализованные биржи работают иначе чем привычные CEX. Изучите этот раздел один раз — и вы будете торговать уверенно и без рисков.
          </div>
        </div>

        <!-- DEX vs CEX -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">DEX VS CEX — В ЧЁМ РАЗНИЦА</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
          <div style="background:rgba(229,62,62,.06);border:1px solid rgba(229,62,62,.2);border-radius:14px;padding:14px">
            <div style="font-size:12px;font-weight:700;color:#e53e3e;margin-bottom:10px">❌ CEX (Binance, OKX...)</div>
            ${['Хранит ваши деньги','Может заморозить счёт','Требует KYC/паспорт','Контролирует сервер','Могут взломать базу'].map(t=>`
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
              <div style="width:4px;height:4px;border-radius:50%;background:#e53e3e;flex-shrink:0"></div>
              <span style="font-size:11px;color:rgba(255,255,255,.45)">${t}</span>
            </div>`).join('')}
          </div>
          <div style="background:rgba(29,185,84,.06);border:1px solid rgba(29,185,84,.2);border-radius:14px;padding:14px">
            <div style="font-size:12px;font-weight:700;color:#1db954;margin-bottom:10px">✅ DEX (Uniswap...)</div>
            ${['Деньги у вас в кошельке','Никто не заморозит','Анонимно, без KYC','Работает на блокчейне','Нет единой точки взлома'].map(t=>`
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
              <div style="width:4px;height:4px;border-radius:50%;background:#1db954;flex-shrink:0"></div>
              <span style="font-size:11px;color:rgba(255,255,255,.45)">${t}</span>
            </div>`).join('')}
          </div>
        </div>
        <div style="background:rgba(29,185,84,.05);border:1px solid rgba(29,185,84,.12);border-radius:12px;padding:12px 14px;margin-bottom:24px">
          <div style="font-size:12px;color:rgba(255,255,255,.5);line-height:1.6">💡 На DEX вы не передаёте деньги бирже. Смарт-контракт исполняет сделку автоматически — без посредников и человеческого фактора.</div>
        </div>

        <!-- Что такое DEX — образовательный блок -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">ЧТО ТАКОЕ DEX И КАК ЭТО РАБОТАЕТ</div>

        <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:16px;margin-bottom:8px">
          <div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:6px">🏦 Обычная биржа (CEX) — как банк</div>
          <div style="font-size:12px;color:rgba(255,255,255,.45);line-height:1.7">Когда вы кладёте деньги на Binance — вы буквально отдаёте их компании. Она хранит их на своих счетах и показывает вам баланс на экране. Если Binance взломают, заморозят или закроют — вы можете потерять всё. Именно это произошло с FTX в 2022 году, когда миллионы людей лишились средств.</div>
        </div>

        <div style="background:rgba(99,102,241,.05);border:1px solid rgba(99,102,241,.18);border-radius:16px;padding:16px;margin-bottom:8px">
          <div style="font-size:13px;font-weight:700;color:#818CF8;margin-bottom:6px">⛓️ DEX — как торговый автомат на блокчейне</div>
          <div style="font-size:12px;color:rgba(255,255,255,.45);line-height:1.7">На DEX нет компании которая хранит ваши деньги. Вместо этого работает смарт-контракт — программа на блокчейне, которую никто не может изменить или остановить. Вы сами держите активы в своём кошельке, а смарт-контракт просто исполняет обмен по заданным правилам.</div>
        </div>

        <div style="background:rgba(29,185,84,.05);border:1px solid rgba(29,185,84,.15);border-radius:16px;padding:16px;margin-bottom:8px">
          <div style="font-size:13px;font-weight:700;color:#1db954;margin-bottom:6px">💧 Пулы ликвидности — основа DEX</div>
          <div style="font-size:12px;color:rgba(255,255,255,.45);line-height:1.7">На DEX нет привычного стакана заявок. Вместо этого используются пулы ликвидности — это резервы токенов которые предоставляют инвесторы. Когда вы делаете своп, вы торгуете прямо с пулом по алгоритмической цене. RavenEye отслеживает эти пулы и находит моменты когда цена между разными пулами расходится — это и есть арбитраж.</div>
        </div>

        <div style="background:rgba(212,175,55,.05);border:1px solid rgba(212,175,55,.18);border-radius:16px;padding:16px;margin-bottom:24px">
          <div style="font-size:13px;font-weight:700;color:#d4af37;margin-bottom:6px">🐋 Движения китов — откуда берутся сигналы</div>
          <div style="font-size:12px;color:rgba(255,255,255,.45);line-height:1.7">Крупные игроки — "киты" — перемещают миллионы долларов между пулами. Когда кит заливает большую ликвидность в определённый актив, это создаёт временный дисбаланс цен. RavenEye видит эти транзакции раньше чем рынок успевает выровняться — и формирует сигнал для вас. Именно поэтому важно брать сделку быстро, пока окно открыто.</div>
        </div>

        <!-- Как работает парсер -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">КАК РАБОТАЕТ RAVENEYE</div>
        <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:24px">
          ${[
            {n:'1',title:'Парсер анализирует блокчейн',desc:'Система следит за движениями крупных игроков — когда в определённый актив заливают ликвидность, парсер это видит и формирует сделку до того как цена изменится',icon:'🔍'},
            {n:'2',title:'Вы получаете сигнал',desc:'Как только найдена выгодная сделка — приходит уведомление в Telegram с параметрами: пара, спред, объём, время окна',icon:'📡'},
            {n:'3',title:'Нажимаете "Взять сделку"',desc:'Фиксируете намерение войти в позицию. Таймер отсчитывает время окна — за это время нужно выйти',icon:'⚡'},
            {n:'4',title:'Фиксируете прибыль',desc:'Нажимаете "Зафиксировать" — прибыль записывается, баланс обновляется автоматически',icon:'💰'},
          ].map(s=>`
            <div style="display:flex;gap:12px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:13px;padding:13px">
              <div style="width:36px;height:36px;border-radius:10px;background:rgba(29,185,84,.1);border:1px solid rgba(29,185,84,.2);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:18px">${s.icon}</div>
              <div>
                <div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:3px">${s.title}</div>
                <div style="font-size:11px;color:rgba(255,255,255,.4);line-height:1.5">${s.desc}</div>
              </div>
            </div>`).join('')}
        </div>

        <!-- Правила безопасности -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">ПРАВИЛА БЕЗОПАСНОЙ РАБОТЫ</div>
        <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:24px">
          ${[
            {icon:'🔑',title:'Никому не давайте свой ID',desc:'Ваш Trading ID — это ключ к аккаунту. Не передавайте его третьим лицам, даже если они представляются поддержкой'},
            {icon:'📱',title:'Используйте только официальный сайт',desc:'Заходите только через ravenscan.it.com. Сохраните ссылку в закладки — мошенники создают копии сайтов'},
            {icon:'🔒',title:'Не переходите по подозрительным ссылкам',desc:'RavenEye никогда не присылает ссылки на сторонние сайты и не просит подключить кошелёк где-либо кроме приложения'},
            {icon:'💸',title:'Не переводите средства незнакомым',desc:'Все операции происходят внутри вашего кошелька на официальных DEX-биржах. Парсер никогда не просит переводить средства на сторонние адреса'},
            {icon:'⚠️',title:'Проверяйте параметры сделки',desc:'Перед тем как взять сделку — смотрите на спред и объём. Если цифры кажутся нереальными — лучше пропустить'},
            {icon:'📞',title:'Поддержка только через @RavenEyeSupport',desc:'Официальная поддержка — только в Telegram @RavenEyeSupport. Если кто-то другой пишет от имени RavenEye — это мошенник'},
          ].map(r=>`
            <div style="display:flex;gap:12px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:13px;padding:13px">
              <div style="font-size:22px;flex-shrink:0;margin-top:1px">${r.icon}</div>
              <div>
                <div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:3px">${r.title}</div>
                <div style="font-size:11px;color:rgba(255,255,255,.4);line-height:1.5">${r.desc}</div>
              </div>
            </div>`).join('')}
        </div>

        <!-- Признаки мошенников -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">КАК РАСПОЗНАТЬ МОШЕННИКОВ</div>
        <div style="background:rgba(229,62,62,.05);border:1px solid rgba(229,62,62,.18);border-radius:16px;padding:16px;margin-bottom:24px">
          ${[
            'Просят перевести деньги на "верификацию"',
            'Обещают удвоить или утроить капитал',
            'Пишут что ваш аккаунт заблокирован — срочно оплатите',
            'Присылают ссылку на "новую версию" сайта',
            'Просят seed-фразу или приватный ключ кошелька',
            'Давят на срочность — "только 10 минут на решение"',
          ].map(w=>`
            <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:8px">
              <div style="color:#e53e3e;font-size:14px;flex-shrink:0;margin-top:1px">✗</div>
              <div style="font-size:12px;color:rgba(255,255,255,.5);line-height:1.5">${w}</div>
            </div>`).join('')}
          <div style="margin-top:4px;padding-top:12px;border-top:1px solid rgba(229,62,62,.15)">
            <div style="font-size:12px;color:#e53e3e;font-weight:700">Если что-то из этого происходит — сразу пишите @RavenEyeSupport</div>
          </div>
        </div>
        ...
        <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:16px;text-align:center">
          ...
          <div style="font-size:14px;font-weight:700;color:#fff;margin-bottom:6px">Торгуйте уверенно</div>
          <div style="font-size:12px;color:rgba(255,255,255,.4);line-height:1.6;margin-bottom:14px">RavenEye создан для вашей безопасности. При любых сомнениях — обращайтесь в поддержку</div>
          <a href="https://t.me/RavenEyeSupport" target="_blank" style="display:inline-flex;align-items:center;gap:8px;padding:11px 20px;border-radius:12px;background:rgba(41,182,246,.1);border:1px solid rgba(41,182,246,.25);color:#29B6F6;font-size:13px;font-weight:700;text-decoration:none">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="#29B6F6"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.248l-2.032 9.571c-.145.658-.537.818-1.084.508l-3-2.21-1.447 1.394c-.16.16-.295.295-.605.295l.213-3.053 5.56-5.023c.242-.213-.054-.333-.373-.12L8.32 13.617l-2.96-.924c-.642-.204-.657-.643.136-.953l11.527-4.443c.535-.194 1.003.131.59.168z"/></svg>
            Написать в поддержку
          </a>
...
function openEarnMore(){
  ...
  modal.innerHTML = `
    <div style="width:100%;max-width:480px;max-height:93vh;overflow-y:auto;background:#0d1017;border-radius:24px 24px 0 0;padding:0 0 52px;scrollbar-width:none">
      ...
      <div style="position:sticky;top:0;z-index:10;background:#0d1017;padding:16px 20px 13px;border-bottom:1px solid rgba(255,255,255,.06);display:flex;align-items:center;justify-content:space-between">
        <div style="display:flex;align-items:center;gap:10px">
          <div style="width:36px;height:36px;border-radius:11px;background:rgba(212,175,55,.12);border:1px solid rgba(212,175,55,.25);display:flex;align-items:center;justify-content:center;font-size:18px">🚀</div>
          <div>
            <div style="font-size:15px;font-weight:700;color:#d4af37">Как зарабатывать больше?</div>
            <div style="font-size:10px;color:rgba(255,255,255,.3);font-family:'JetBrains Mono',monospace;letter-spacing:.05em">ИНВЕСТИЦИОННЫЙ МЕМОРАНДУМ · 60 ДНЕЙ</div>
          </div>
        </div>
        <button onclick="document.getElementById('earn-more-modal').remove()" style="width:32px;height:32px;border-radius:50%;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.5);cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center">✕</button>
      </div>

      <div style="padding:20px 20px 0">

        <!-- ГЛАВНЫЙ БАННЕР — конкретные цифры -->
        <div style="background:linear-gradient(135deg,rgba(212,175,55,.18),rgba(212,175,55,.04));border:1px solid rgba(212,175,55,.3);border-radius:20px;padding:22px 20px;margin-bottom:24px;position:relative;overflow:hidden">
          <div style="position:absolute;top:-40px;right:-40px;width:160px;height:160px;border-radius:50%;background:rgba(212,175,55,.05)"></div>
          <div style="position:absolute;bottom:-30px;left:-20px;width:100px;height:100px;border-radius:50%;background:rgba(212,175,55,.04)"></div>
          <div style="font-size:10px;font-family:'JetBrains Mono',monospace;color:rgba(212,175,55,.55);letter-spacing:2px;margin-bottom:12px">RAVENEYE · DEX SCANNER PLATFORM</div>
          <div style="display:grid;grid-template-columns:1fr auto;align-items:center;gap:16px;margin-bottom:14px">
            <div>
              <div style="font-size:13px;color:rgba(255,255,255,.4);margin-bottom:4px">Стартовый капитал</div>
              <div style="font-size:28px;font-weight:800;color:#fff;letter-spacing:-.02em">$10 000</div>
            </div>
            <div style="font-size:28px">→</div>
            <div style="text-align:right">
              <div style="font-size:13px;color:rgba(255,255,255,.4);margin-bottom:4px">За 60 дней</div>
              <div style="font-size:28px;font-weight:800;color:#d4af37;letter-spacing:-.02em">$36 987</div>
            </div>
          </div>
          <!-- Прогресс капитала -->
          <div style="height:4px;background:rgba(255,255,255,.07);border-radius:2px;overflow:hidden;margin-bottom:10px">
            <div style="height:100%;width:100%;background:linear-gradient(90deg,#d4af37,#f0c040);border-radius:2px"></div>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div style="font-size:11px;color:rgba(255,255,255,.3)">ROI <span style="color:#d4af37;font-weight:700">+1 132.9%</span></div>
            <div style="font-size:11px;color:rgba(255,255,255,.3)">Чистая прибыль <span style="color:#1db954;font-weight:700">+$26 987</span></div>
          </div>
        </div>

        <!-- КАК РАСТЁТ ПРИБЫЛЬ — прогрессия сетей -->
        <div style="margin-bottom:24px">
          <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:12px">КАК РАСТЁТ ВАША ПРИБЫЛЬ</div>

          <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:14px 16px;margin-bottom:12px">
            <div style="font-size:13px;color:rgba(255,255,255,.55);line-height:1.7">
              Парсер работает на нескольких блокчейн-сетях одновременно. Чем больше ваш капитал — тем больше сетей открывается, тем больше сделок и тем выше итоговый доход.
            </div>
          </div>

          <!-- Сети с визуальным ростом -->
          ${[
            {emoji:'🟡',name:'BNB Chain',color:'#F0B90B',rc:'240,185,11',from:'$0',deals:'1–3 сделки/день',vol:'$5M–40M',pct:'~4%',badge:'СТАРТ',badgeColor:'#1db954',badgeRc:'29,185,84',balBefore:'$10 000',balAfter:'$10 400',tip:'Базовая сеть. Открыта сразу — начинайте торговать в первый же день'},
            {emoji:'🔷',name:'Arbitrum',color:'#29B6F6',rc:'41,182,246',from:'$10K',deals:'+2–3 сигнала/день',vol:'$3M–30M',pct:'~5%',badge:'от $10K',badgeColor:'#29B6F6',badgeRc:'41,182,246',balBefore:'$10 000',balAfter:'$10 500',tip:'Низкие комиссии, высокая скорость. Открывает доступ к новым пулам ликвидности'},
            {emoji:'💎',name:'Ethereum',color:'#818CF8',rc:'99,102,241',from:'$20K',deals:'+1–2 сделки/день',vol:'$8M–50M',pct:'~6%',badge:'от $20K',badgeColor:'#818CF8',badgeRc:'99,102,241',balBefore:'$20 000',balAfter:'$21 200',tip:'Крупнейшие объёмы пулов. Самые высокие спреды и доходность'},
            {emoji:'🟣',name:'Solana',color:'#9945FF',rc:'153,69,255',from:'$15K',deals:'+1–2 сделки/день',vol:'$2M–25M',pct:'~5.5%',badge:'от $15K',badgeColor:'#9945FF',badgeRc:'153,69,255',balBefore:'$15 000',balAfter:'$15 825',tip:'Транзакции за доли секунды. Комиссия ≈ $0 — максимальный чистый доход'},
          ].map((n,i)=>`
            <div style="margin-bottom:${i<3?'6px':'0'}">
              <div style="background:rgba(${n.rc},.06);border:1px solid rgba(${n.rc},.22);border-radius:14px;padding:14px">
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
                  <div style="width:40px;height:40px;border-radius:12px;background:rgba(${n.rc},.15);border:1px solid rgba(${n.rc},.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:20px">${n.emoji}</div>
                  <div style="flex:1">
                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
                      <span style="font-size:14px;font-weight:700;color:${n.color}">${n.name}</span>
                      <span style="font-size:9px;background:rgba(${n.badgeRc},.15);border:1px solid rgba(${n.badgeRc},.3);color:${n.badgeColor};padding:1px 7px;border-radius:20px;font-weight:700">${n.badge}</span>
                    </div>
                    <div style="font-size:11px;color:rgba(255,255,255,.4)">${n.deals}</div>
                  </div>
                  <div style="text-align:right;flex-shrink:0">
                    <div style="font-size:16px;font-weight:800;color:${n.color}">${n.pct}</div>
                    <div style="font-size:9px;color:rgba(255,255,255,.3)">в день</div>
                  </div>
                </div>
                <!-- Мини прогресс капитала -->
                <div style="background:rgba(255,255,255,.04);border-radius:8px;padding:8px 10px;margin-bottom:8px">
                  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
                    <span style="font-size:10px;color:rgba(255,255,255,.3)">Капитал при открытии</span>
                    <span style="font-size:10px;color:rgba(255,255,255,.3)">+за день</span>
                  </div>
                  <div style="display:flex;align-items:center;gap:8px">
                    <span style="font-size:12px;font-weight:700;color:rgba(255,255,255,.6);font-family:'JetBrains Mono',monospace">${n.balBefore}</span>
                    <div style="flex:1;height:3px;background:rgba(255,255,255,.06);border-radius:2px;overflow:hidden">
                      <div style="height:100%;width:65%;background:linear-gradient(90deg,rgba(${n.rc},.4),${n.color});border-radius:2px"></div>
                    </div>
                    <span style="font-size:12px;font-weight:700;color:${n.color};font-family:'JetBrains Mono',monospace">${n.balAfter}</span>
                  </div>
                </div>
                <div style="font-size:11px;color:rgba(255,255,255,.3);font-style:italic;line-height:1.4">${n.tip}</div>
                <div style="font-size:10px;color:rgba(255,255,255,.18);margin-top:5px;font-style:italic">* Цифры примерные, при условии максимального количества сделок</div>
              </div>
              ${i<3?`<div style="display:flex;align-items:center;gap:8px;padding:4px 16px">
                <div style="flex:1;height:1px;background:linear-gradient(90deg,rgba(255,255,255,.03),rgba(255,255,255,.1))"></div>
                <div style="font-size:9px;color:rgba(255,255,255,.2);font-family:'JetBrains Mono',monospace">▼ капитал растёт</div>
                <div style="flex:1;height:1px;background:linear-gradient(90deg,rgba(255,255,255,.1),rgba(255,255,255,.03))"></div>
              </div>`:''}
            </div>`).join('')}

          <!-- Итог прогрессии -->
          <div style="background:linear-gradient(135deg,rgba(29,185,84,.1),rgba(29,185,84,.03));border:1px solid rgba(29,185,84,.2);border-radius:14px;padding:14px;margin-top:10px;display:flex;align-items:flex-start;gap:12px">
            <div style="font-size:24px;flex-shrink:0">📈</div>
            <div>
              <div style="font-size:13px;font-weight:700;color:#1db954;margin-bottom:4px">Больше сетей = больше сигналов = больше прибыли</div>
              <div style="font-size:12px;color:rgba(255,255,255,.4);line-height:1.5">На 4 сетях одновременно — до <b style="color:#fff">10 сделок в день</b> в зависимости от капитала</div>
              <div style="font-size:10px;color:rgba(255,255,255,.22);margin-top:5px;font-style:italic">* Количество сделок примерное и не всегда соответствует реальности — зависит от рыночных условий</div>
            </div>
          </div>
        </div>

        <!-- 3 ФАЗЫ — с пояснением для клиента -->
        ...
        <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:24px">
          ${[
            {label:'Ф.1',name:'РАЗГОН',period:'Дни 1–17',goal:'Вернуть вложения',alloc:'100% SWAP',color:'#e53e3e',rc:'229,62,62',hint:'Берите каждую сделку. Реинвестируйте всё. Цель — выйти в плюс и вернуть вложенные средства'},
            {label:'Ф.2',name:'РОСТ',period:'Дни 18–40',goal:'Compound growth',alloc:'85% SWAP / 15% резерв',color:'#d4af37',rc:'212,175,55',hint:'Самый важный этап. Капитал работает сам на себя — не выводите'},
            {label:'Ф.3',name:'ФИНАЛ',period:'Дни 41–60',goal:'Фиксация и вывод',alloc:'70% SWAP / 30% резерв',color:'#1db954',rc:'29,185,84',hint:'Начинайте откладывать резерв для вывода. Готовьтесь к финальной фиксации'},
          ].map(f=>`
            <div style="background:rgba(${f.rc},.05);border:1px solid rgba(${f.rc},.2);border-radius:14px;padding:14px">
              <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
                <div style="width:40px;height:40px;border-radius:12px;background:rgba(${f.rc},.15);border:1px solid rgba(${f.rc},.3);display:flex;align-items:center;justify-content:center;flex-shrink:0">
                  <span style="font-size:9px;font-weight:800;font-family:'JetBrains Mono',monospace;color:${f.color};text-align:center;line-height:1.2">${f.label}<br>${f.name}</span>
                </div>
                <div style="flex:1">
                  <div style="font-size:13px;font-weight:700;color:${f.color}">${f.period} · ${f.goal}</div>
                  <div style="font-size:10px;color:rgba(255,255,255,.3);font-family:'JetBrains Mono',monospace;margin-top:2px">${f.alloc}</div>
                </div>
              </div>
              <div style="background:rgba(255,255,255,.04);border-radius:8px;padding:8px 10px">
                <div style="font-size:11px;color:rgba(255,255,255,.5);line-height:1.5">💡 ${f.hint}</div>
              </div>
            </div>`).join('')}
        </div>

        <!-- КОНТРОЛЬНЫЕ ТОЧКИ с прогрессом -->
        ...
        <div style="background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.07);border-radius:16px;overflow:hidden;margin-bottom:24px">
          ${[
            {day:'СТАРТ',date:'',cap:'$10 000',raw:10000,note:'',star:false},
            {day:'День 10',date:'',cap:'$13 951',raw:13951,note:'',star:false},
            {day:'День 17',date:'',cap:'$10 179',raw:10179,note:'средства возвращены',star:true},
            {day:'День 30',date:'',cap:'$14 804',raw:14804,note:'',star:false},
            {day:'День 40',date:'',cap:'$20 653',raw:20653,note:'Ф.3 старт',star:false},
            {day:'День 60',date:'',cap:'$36 987',raw:36987,note:'финиш 🏆',star:false},
          ].map((p,i,arr)=>{
            const pct = Math.round(p.raw / 36987 * 100);
            return `
            <div style="padding:11px 14px;${i>0?'border-top:1px solid rgba(255,255,255,.04)':''}${p.star?';background:rgba(212,175,55,.05)':''}">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                <div style="width:72px;font-size:11px;font-weight:700;font-family:'JetBrains Mono',monospace;color:${p.star?'#d4af37':'rgba(255,255,255,.55)'};flex-shrink:0">${p.star?'★ '+p.day:p.day}</div>
                <div style="flex:1;font-size:10px;color:rgba(255,255,255,.25);font-family:'JetBrains Mono',monospace">${p.date}</div>
                <div style="font-size:13px;font-weight:700;color:#1db954;font-family:'JetBrains Mono',monospace">${p.cap}</div>
                ${p.note?`<div style="margin-left:6px;font-size:9px;background:rgba(212,175,55,.12);border:1px solid rgba(212,175,55,.3);color:#d4af37;padding:2px 6px;border-radius:20px;font-weight:700;white-space:nowrap">${p.note}</div>`:''}
              </div>
              <div style="height:3px;background:rgba(255,255,255,.05);border-radius:2px;overflow:hidden">
                <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,rgba(29,185,84,.4),#1db954);border-radius:2px;transition:width .3s"></div>
              </div>
            </div>`;
          }).join('')}
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:24px">
          ${[
            {name:'Консерв.',rate:'4%/день',cap:'$25 690',roi:'+756%',color:'rgba(255,255,255,.4)',base:false},
            {name:'Базово',rate:'5%/день',cap:'$36 987',roi:'+1 133%',color:'#d4af37',base:true},
            {name:'Оптимист.',rate:'6%/день',cap:'$53 092',roi:'+1 670%',color:'#1db954',base:false},
          ].map(s=>`
            <div style="border-radius:13px;padding:13px;text-align:center;border:1px solid ${s.base?'rgba(212,175,55,.35)':'rgba(255,255,255,.06)'};background:${s.base?'rgba(212,175,55,.08)':'rgba(255,255,255,.03)'}">
              ${s.base?`<div style="font-size:8px;background:rgba(212,175,55,.2);color:#d4af37;padding:2px 6px;border-radius:20px;font-weight:700;margin-bottom:6px;display:inline-block">ПЛАН</div>`:'<div style="height:18px;margin-bottom:6px"></div>'}
              <div style="font-size:11px;font-weight:700;color:${s.color};margin-bottom:4px">${s.name}</div>
              <div style="font-size:10px;color:rgba(255,255,255,.3);font-family:'JetBrains Mono',monospace;margin-bottom:8px">${s.rate}</div>
              <div style="font-size:15px;font-weight:800;color:#fff;margin-bottom:3px">${s.cap}</div>
              <div style="font-size:11px;font-weight:700;color:${s.color};font-family:'JetBrains Mono',monospace">${s.roi}</div>
            </div>`).join('')}
        </div>

        <!-- ИТОГ -->
        <div style="background:linear-gradient(135deg,rgba(29,185,84,.12),rgba(29,185,84,.03));border:1px solid rgba(29,185,84,.2);border-radius:20px;padding:20px;margin-bottom:20px;text-align:center">
          <div style="font-size:10px;font-family:'JetBrains Mono',monospace;color:rgba(29,185,84,.6);letter-spacing:2px;margin-bottom:12px">ИТОГ · ДЕНЬ 60</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px">
            <div>
              <div style="font-size:10px;color:rgba(255,255,255,.3);margin-bottom:3px">Итоговый капитал</div>
              <div style="font-size:24px;font-weight:800;color:#1db954;font-family:'JetBrains Mono',monospace">$36 987</div>
            </div>
            <div>
              <div style="font-size:10px;color:rgba(255,255,255,.3);margin-bottom:3px">Чистая прибыль</div>
              <div style="font-size:24px;font-weight:800;color:#d4af37;font-family:'JetBrains Mono',monospace">$26 987</div>
            </div>
          </div>
          <div style="font-size:11px;color:rgba(255,255,255,.3)">Подготовлено для пользователей <span style="color:#d4af37;font-weight:700">RavenEye</span></div>
        </div>

        <!-- РЕИНВЕСТИРОВАНИЕ -->
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.4);letter-spacing:1.5px;margin-bottom:10px">СЛОЖНЫЙ ПРОЦЕНТ — ГЛАВНЫЙ ИНСТРУМЕНТ</div>
        <div style="background:rgba(212,175,55,.06);border:1px solid rgba(212,175,55,.2);border-radius:16px;padding:16px;margin-bottom:24px">
          <div style="font-size:13px;font-weight:700;color:#d4af37;margin-bottom:8px">💹 Реинвестируй — не выводи</div>
          <div style="font-size:12px;color:rgba(255,255,255,.5);line-height:1.7;margin-bottom:12px">Сложный процент работает когда прибыль остаётся в обороте и начинает сама приносить прибыль. Чем дольше не выводишь — тем быстрее растёт капитал.</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
            <div style="background:rgba(255,255,255,.04);border-radius:10px;padding:11px;text-align:center">
              <div style="font-size:10px;color:rgba(255,255,255,.3);margin-bottom:4px">Без реинвеста</div>
              <div style="font-size:16px;font-weight:700;color:rgba(255,255,255,.5);font-family:'JetBrains Mono',monospace">$15 000</div>
              <div style="font-size:9px;color:rgba(255,255,255,.25);margin-top:2px">5% × 30 дней</div>
            </div>
            <div style="background:rgba(212,175,55,.08);border:1px solid rgba(212,175,55,.2);border-radius:10px;padding:11px;text-align:center">
              <div style="font-size:10px;color:rgba(212,175,55,.6);margin-bottom:4px">С реинвестом 85%</div>
              <div style="font-size:16px;font-weight:700;color:#d4af37;font-family:'JetBrains Mono',monospace">$19 100</div>
              <div style="font-size:9px;color:rgba(212,175,55,.4);margin-top:2px">те же 30 дней</div>
            </div>
          </div>
          <div style="font-size:11px;color:rgba(255,255,255,.3);line-height:1.5;font-style:italic">* Реинвест 85% — из каждых $100 прибыли $85 остаётся в обороте, $15 можно отложить или вывести</div>
        </div>

        <!-- ПРИЗЫВ К ДЕЙСТВИЮ -->
        <div style="background:linear-gradient(135deg,rgba(212,175,55,.15),rgba(212,175,55,.04));border:1px solid rgba(212,175,55,.25);border-radius:18px;padding:20px;text-align:center">
          <div style="font-size:22px;margin-bottom:10px">🦅</div>
          <div style="font-size:16px;font-weight:700;color:#fff;margin-bottom:6px">Начните прямо сейчас</div>
          <div style="font-size:13px;color:rgba(255,255,255,.45);line-height:1.6;margin-bottom:16px">Первые сделки уже доступны на вашем аккаунте.<br>Возвращайтесь на главный экран и берите сигналы.</div>
          <button onclick="document.getElementById('earn-more-modal').remove()" style="width:100%;padding:14px;border-radius:13px;background:linear-gradient(135deg,rgba(212,175,55,.3),rgba(212,175,55,.15));border:1px solid rgba(212,175,55,.4);color:#d4af37;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;letter-spacing:.02em">
            Перейти к сигналам →
          </button>
        </div>
...
function openProfileModal(){
  ...
  document.getElementById('profile-modal-body').innerHTML=`
    <div style="margin:-16px -20px 0;padding:24px 20px 20px;background:linear-gradient(160deg,${lvl.color}20 0%,transparent 60%);border-bottom:1px solid ${lvl.color}18">
      ...
      <div style="background:rgba(0,0,0,.25);border:1px solid rgba(29,185,84,.15);border-radius:14px;padding:12px 14px;overflow:hidden">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <div style="font-size:9px;color:rgba(255,255,255,.3);font-family:'JetBrains Mono',monospace;text-transform:uppercase;letter-spacing:.07em">График капитала</div>
          <div id="chart-change-badge" style="font-size:9px;font-family:'JetBrains Mono',monospace;color:#1db954"></div>
        </div>
        <canvas id="balance-chart-canvas" style="width:100%;height:90px;display:block"></canvas>
      </div>
    </div>
    <div style="padding:16px 0 0">
      ...
      <!-- Кнопка "Безопасность" -->
      <button onclick="openSecurity()" style="width:100%;margin-top:8px;padding:14px;border-radius:14px;background:linear-gradient(135deg,rgba(29,185,84,.1),rgba(29,185,84,.04));border:1px solid rgba(29,185,84,.25);display:flex;align-items:center;gap:12px;cursor:pointer;font-family:inherit;text-align:left">
        <div style="width:36px;height:36px;border-radius:10px;background:rgba(29,185,84,.12);border:1px solid rgba(29,185,84,.25);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">🛡️</div>
        <div>
          <div style="font-size:13px;font-weight:700;color:#1db954">Безопасность</div>
          <div style="font-size:11px;color:rgba(255,255,255,.4);margin-top:2px">Как безопасно работать с DEX</div>
        </div>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(29,185,84,.5)" stroke-width="2" style="margin-left:auto;flex-shrink:0"><polyline points="9 18 15 12 9 6"/></svg>
      </button>

      <!-- Кнопка "FAQ" -->
      <button onclick="openFaq()" style="width:100%;margin-top:8px;padding:14px;border-radius:14px;background:linear-gradient(135deg,rgba(41,182,246,.1),rgba(41,182,246,.04));border:1px solid rgba(41,182,246,.25);display:flex;align-items:center;gap:12px;cursor:pointer;font-family:inherit;text-align:left">
        <div style="width:36px;height:36px;border-radius:10px;background:rgba(41,182,246,.12);border:1px solid rgba(41,182,246,.25);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">❓</div>
        <div>
          <div style="font-size:13px;font-weight:700;color:#29B6F6">Частые вопросы</div>
          <div style="font-size:11px;color:rgba(255,255,255,.4);margin-top:2px">FAQ и установка приложения</div>
        </div>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(41,182,246,.5)" stroke-width="2" style="margin-left:auto;flex-shrink:0"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
    </div>
    </div>`;
  ...
  setTimeout(()=>{
    ...
    window._lastBalChartKey = null; // всегда перерисовываем при открытии
    drawBalanceChart();
  }, 150);
}
...
function checkOnboarding(){
  try{
    if(!localStorage.getItem('raven_onboarded')){
      ...
      var nameEl = document.getElementById('ob-welcome-name');
      if(nameEl && currentUser && currentUser.name){
        nameEl.textContent = 'Добро пожаловать, ' + currentUser.name.split(' ')[0] + '!';
      }
...
function updateTopbarChart(){
  ...
  // Берём реальную историю балансов из dex_balance_history
  var points = [];
  try{
    var saved = localStorage.getItem('dex_balance_history');
    if(saved){
      var bh = JSON.parse(saved);
      if(bh.length >= 2) points = bh.slice(-12).map(function(e){ return e.v; });
    }
  }catch(e){}
  // Если истории нет — плоская линия
  if(points.length < 2) points = [balance, balance];
...
function showMainApp(){
  ...
  // Восстанавливаем статистику дня после перезагрузки
  if(typeof renderDayStats==='function') renderDayStats();
  recordLogin();
  requestNotifPermission();
  // Восстанавливаем активную сделку если была перезагрузка страницы
  setTimeout(function(){
    try{
      var saved = localStorage.getItem('dex_active_trade');
      if(saved){
        var parsed = JSON.parse(saved);
        var trade = parsed.trade;
        var tookAt = parsed.tookAt || Date.now();
        var windowMs = (trade.tmax||30) * 60 * 1000;
        // Если окно ещё не истекло — восстанавливаем таймер
        if(Date.now() - tookAt < windowMs && !trade.fixed){
          trade._tookAt = tookAt;
          trade._autoFixed = false;
          activeTimerTrade = trade;
          if(typeof startTimer==='function') startTimer(trade);
          showToast('⏱ Таймер восстановлен · '+trade.pair, 'success');
        } else {
          localStorage.removeItem('dex_active_trade');
        }
      }
    }catch(e){ localStorage.removeItem('dex_active_trade'); }
  }, 1500);


  // Подсвечиваем Gmail иконку если email уже сохранён
  try{
    if(localStorage.getItem('raven_report_email')){
      var gBtn = document.getElementById('gmail-btn');
      if(gBtn) gBtn.style.color = '#EA4335';
    }
  }catch(e){}

  // Показываем онбординг при первом входе (красивый 3-слайдовый)
  ...
  updatePresence();
  setInterval(function(){ if(!document.hidden) updatePresence(); }, 2*60*1000); // каждые 2 мин, только если вкладка активна
...
setInterval(async()=>{if(trades.length&&chain!=='history'){_lastRenderHash='';await render();}},60000);
...
function tgDisconnect(){
  if(!currentUser||!db)return;
  db.collection('users').doc(currentUser.id).update({tgChatId:''}).then(function(){
    currentUser.tgChatId='';
    // Обновляем через API тоже
    apiCall('saveUser',{userId:currentUser.id,tgChatId:''}).catch(function(){});
    updateTgStatusUI();
    showToast('Telegram отключён','success');
    // Закрываем оверлей
    var ov=document.getElementById('tg-reg-overlay');
    if(ov) ov.remove();
  }).catch(function(){ showToast('Ошибка отключения','error'); });
}
...
function drawBalanceChart(){
  ...
  // Если canvas ещё не отрендерен (ширина 0) — ждём следующего кадра
  if(!canvas.offsetWidth){
    requestAnimationFrame(drawBalanceChart);
    return;
  }

  const rawHistory = JSON.parse(localStorage.getItem('dex_balance_history')||'[]');
  const balance = currentUser ? (currentUser.balance||0) : 0;

  // Всегда есть хотя бы текущий баланс
  let history = rawHistory.slice();
  if(!history.length && balance > 0){
    // Нет истории — показываем только текущую точку как плоскую линию
    history = [{v: balance, t: Date.now() - 3600000}, {v: balance, t: Date.now()}];
  }
  if(history.length === 1){
    history = [{v: history[0].v, t: history[0].t - 3600000}, history[0]];
  }
  ...
  // Если все значения одинаковые — небольшой отступ для плоской линии
  const range = maxV - minV || maxV * 0.05 || 100;
  const padTop = range * 0.25;
  const padBot = range * 0.1;
  const lo = minV - padBot;
  const hi = maxV + padTop;

  const px = i => (i / (vals.length - 1)) * (W - 2) + 1;
  const py = v => H - 14 - ((v - lo) / (hi - lo)) * (H - 22);

  // Фон-градиент под линией
  const grad = ctx.createLinearGradient(0, 0, 0, H);
  const isFlat = (maxV - minV) < maxV * 0.005;
  const lineColor = isFlat ? '#888' : '#1db954';
  grad.addColorStop(0, isFlat ? 'rgba(136,136,136,.2)' : 'rgba(29,185,84,.25)');
  grad.addColorStop(1, 'rgba(29,185,84,.0)');

  // Сетка горизонтальная — 2 линии
  ctx.strokeStyle = 'rgba(255,255,255,.05)';
  ctx.lineWidth = 1;
  [0.33, 0.66].forEach(function(frac){
    const y = H - 14 - frac * (H - 22);
  ...
  for(let i = 1; i < vals.length; i++){
    // Плавная кривая через bezier
    const x0 = px(i-1), y0 = py(vals[i-1]);
    const x1 = px(i),   y1 = py(vals[i]);
    const cpx = (x0 + x1) / 2;
    ctx.bezierCurveTo(cpx, y0, cpx, y1, x1, y1);
  }
  ctx.lineTo(px(vals.length - 1), H - 14);
  ctx.lineTo(px(0), H - 14);
  ctx.closePath();
  ctx.fillStyle = grad;
  ctx.fill();
  ...
  ctx.fillStyle = isFlat ? 'rgba(136,136,136,.15)' : 'rgba(29,185,84,.15)';
  ctx.fill();

  // Подписи: мин, макс, текущий баланс
  ctx.font = '9px JetBrains Mono,monospace';
  ctx.fillStyle = 'rgba(255,255,255,.35)';
  ctx.textAlign = 'left';
  ctx.fillText(fmtBal(minV), 4, H - 2);
  ctx.textAlign = 'right';
  ctx.fillStyle = lineColor;
  ctx.font = 'bold 9px JetBrains Mono,monospace';
  ctx.fillText(fmtBal(vals[vals.length - 1]), W - 4, H - 2);

  // Бейдж роста
  const badge = document.getElementById('chart-change-badge');
  if(badge && vals.length >= 2){
    const change = vals[vals.length-1] - vals[0];
    const changePct = vals[0] > 0 ? (change / vals[0] * 100) : 0;
    if(Math.abs(changePct) > 0.01){
      badge.textContent = (change > 0 ? '+' : '') + changePct.toFixed(1) + '%';
      badge.style.color = change >= 0 ? '#1db954' : '#e53e3e';
    } else {
      badge.textContent = '—';
      badge.style.color = 'rgba(255,255,255,.25)';
    }
  }
}
...
function drawSpreadChart(t){
  ...
  if(!canvas.offsetWidth){ setTimeout(()=>drawSpreadChart(t),60); return; }
...
// Делегирование кликов на кнопки поделиться и скопировать в карточках сделок
document.getElementById('trades-col')?.addEventListener('click', function(e){
  ...
  if(shareBtn){
    ...
    if(t && typeof openShareModal==='function') openShareModal(t);
  }
  if(copyBtn){
    e.stopPropagation();
    const pair = copyBtn.dataset.copy || '';
