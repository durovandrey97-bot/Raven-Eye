...
function getAddedTime(t){if(t.createdAtMs)return new Date(t.createdAtMs);if(!t.createdAt)return null;if(t.createdAt.toDate)return t.createdAt.toDate();if(t.createdAt.seconds)return new Date(t.createdAt.seconds*1000);if(typeof t.createdAt==='string')return new Date(t.createdAt);return null;}
...
function isExpired(t){const d=getAddedTime(t);if(!d)return false;const windowMs=((t.tmax||30)+5)*60*1000;return (Date.now()-d.getTime())>windowMs;}
...
function updateStats(list){
  // Считаем только не истёкшие сделки
  var active = list.filter(function(t){ return !isExpired(t); });
  animateStat('st-pairs', active.length, {suffix:'',decimals:0});
  if(!active.length){['st-avg','st-best','st-vol'].forEach(id=>{const el=document.getElementById(id);if(el){el.textContent='—';_statPrev[id]='—';}});return;}
  const avg=parseFloat((active.reduce((s,t)=>s+(t.lo+t.hi)/2,0)/active.length).toFixed(2));
  animateStat('st-avg', avg, {suffix:'%',decimals:2});
  animateStat('st-best', Math.max(...active.map(t=>t.hi)), {suffix:'%',decimals:2});
  const volStr=fmtVol(active.reduce((s,t)=>s+(t.vol||0),0));
