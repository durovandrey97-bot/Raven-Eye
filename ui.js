// ═══ ui.js — весь UI: render, модалы, графики, анимации ═══

import { CHAIN_META, TOKEN_ICONS, CHAIN_GRADIENT, DEX_LINKS,
         NET_THRESHOLDS, NET_COLORS, NET_LABELS, SITE_URL, CIRCUMFERENCE,
         PROFIT_HISTORY_KEY }                                    from './config.js';
import { trades, currentUser, db, selectedId, chain, sortMode, sortDir,
         minSpread, minVol, filterDex, spreadHistory, hiddenNets,
         profitHistory, alerts, alertsOn, sessionStats, tradeHistory,
         knownIds, seenTradeIds, activeTimerTrade, timerInterval,
         setSelectedId, setAlertsOn, saveProfitEntry, addToHistory,
         saveBalanceHistory, loadAlerts, saveAlerts, recordLogin }  from './state.js';
import { tgSendToUser, tgRegisterUser, loadThresholds,
         subscribeToTrades }                                     from './firebase.js';

export { fmtVol, fmtBal, isExpired, getAddedTime, hasRealTime,
         getPoolName, getTokenIcon, chainBadge, applyProfileLevel,
         showSkeleton, render, openDetailModal, openProfileModal,
         showToast, launchConfetti, notifyFixed, updateConnectionStatus,
         showMainApp, showUserDeletedBanner, resetDataTimer,
         drawSpreadChart, drawBalanceChart, updateMobAlertBadge,
         addAlert, renderAlerts, calcNet, updateStats };
...
async function render(){
  ...
  col.innerHTML=list.map((t,i)=>{
    ...
    if(!expired&&addedDate&&hasTime){
      ...
      cdHtml=`<div class="countdown ${cls}" data-id="${t._id}"><span class="cd-time">⏱ ${Math.floor(rem/60000)}:${String(Math.floor((rem%60000)/1000)).padStart(2,'0')}</span><div class="cd-bar"><div class="cd-fill" style="width:${pct}%;background:${col2}"></div></div></div>`;
    ...
    return `<div class="trade-item-wrap" data-wrap-id="${t._id}">
      ...
      <div class="trade-item${sel?' selected':''}${expired?' expired':''}${locked?' locked':''}${t.fixed?' fixed-trade':''}" style="animation-delay:${i*.04}s;border-left:4px solid ${t.fixed?'#14b8a6':borderColor};background-image:linear-gradient(135deg,${t.fixed?'rgba(20,184,166,.06)':chainGrad} 0%,transparent 55%);${chainColorVar}${fixedColorVar}" data-id="${t._id}">
      ${lockHtml}
      <div class="ti-top">
...
const notifiedFixed = new Set(JSON.parse(sessionStorage.getItem('dex_notified_fixed')||'[]'));
