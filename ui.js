...
function updateStats(list){
  ...
  // Обновляем pill-статистику
  const pillPairs=document.getElementById('pill-pairs');
  const pillAvg=document.getElementById('pill-avg');
  const pillBest=document.getElementById('pill-best');
  const pillVol=document.getElementById('pill-vol');
  if(pillPairs) pillPairs.textContent=list.length;
  if(pillAvg) pillAvg.textContent=avg.toFixed(2)+'%';
  if(pillBest) pillBest.textContent=Math.max(...list.map(t=>t.hi)).toFixed(2)+'%';
  if(pillVol) pillVol.textContent=volStr;
}
...
async function render(){
  ...
  const borderW=expired?'3px':Math.max(3,Math.min(8,Math.round((t.hi||0)*2.5)))+'px';
    ...
    // lock
    let lockHtml='';
    if(locked){const def=threshold-userBalance,pct=Math.min(100,(userBalance/threshold)*100);lockHtml=`<div class="lock-overlay"><div class="lock-icon">🔒</div><div class="lock-txt">НЕДОСТАТОЧНЫЙ КАПИТАЛ</div><div class="lock-sub">для входа в эту сеть</div><div class="lock-deficit">не хватает ${def>=1000?'$'+(def/1000).toFixed(1)+'K':'$'+def}</div><div class="lock-prog"><div class="lock-prog-fill" style="width:${pct}%"></div></div></div>`;}
...
function openProfileModal(){
  ...
  // Рамка аватара по активности
  const av2 = document.getElementById('profile-avatar');
  if(av2){
    const lastTrade = profitHistory[0];
    const lastTradeTime = lastTrade ? lastTrade.time : 0;
    const hourAgo = Date.now() - 60*60*1000;
    const todayEarned = profitHistory.filter(function(e){
      return new Date(e.time).toDateString()===new Date().toDateString();
    }).reduce(function(s,e){return s+e.amount;},0);
    if(Date.now()-lastTradeTime < 60*60*1000){
      // Зелёная — была сделка за последний час
      av2.style.border='2px solid #1db954';
      av2.style.boxShadow='0 0 12px rgba(29,185,84,.4)';
    } else if(todayEarned > 0){
      // Золотая — сегодня заработал
      av2.style.border='2px solid #d4af37';
      av2.style.boxShadow='0 0 12px rgba(212,175,55,.4)';
    } else {
      av2.style.border='2px solid '+lvl.color+'55';
      av2.style.boxShadow='none';
    }
  }
  document.getElementById('profile-modal').classList.add('show');
  setTimeout(()=>{saveBalanceHistory(balance);drawBalanceChart();},80);
}
...
function openDetailModal(t){
  ...
  document.getElementById('detail-modal-body').innerHTML=`
    ...
    <!-- Вкладки детального модала -->
    <div style="display:flex;gap:2px;margin-bottom:14px;background:rgba(255,255,255,.05);border-radius:10px;padding:3px">
      <button id="dtab-overview" onclick="window._switchDTab('overview')" style="flex:1;padding:7px;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;font-family:'Space Grotesk',sans-serif;background:var(--bg3);color:var(--txt);transition:all .15s">Обзор</button>
      <button id="dtab-chart" onclick="window._switchDTab('chart')" style="flex:1;padding:7px;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;font-family:'Space Grotesk',sans-serif;background:transparent;color:var(--txt3);transition:all .15s">График</button>
      <button id="dtab-similar" onclick="window._switchDTab('similar')" style="flex:1;padding:7px;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;font-family:'Space Grotesk',sans-serif;background:transparent;color:var(--txt3);transition:all .15s">Похожие ${similar.length?'('+similar.length+')':''}</button>
    </div>
    <!-- Вкладка Обзор -->
    <div id="dtab-content-overview">
    ...
    <!-- Вкладка График -->
    <div id="dtab-content-chart" style="display:none">
  ...
  window._switchDTab=function(tab){
    ['overview','chart','similar'].forEach(function(name){
      var btn=document.getElementById('dtab-'+name);
      var content=document.getElementById('dtab-content-'+name);
      var active=name===tab;
      if(btn){btn.style.background=active?'var(--bg3)':'transparent';btn.style.color=active?'var(--txt)':'var(--txt3)';}
      if(content) content.style.display=active?'block':'none';
    });
    if(tab==='chart') setTimeout(()=>drawSpreadChart(window._detailModalTrade),80);
  };
...
function notifyFixed(t){
  ...
  // Browser notification (Safari не поддерживает — проверяем)
  try{
...
function renderDayStats() {
  ...
  var el = document.getElementById('day-signals');
  if (el) el.textContent = dayStats.signals;
  el = document.getElementById('day-best');
  if (el) el.textContent = dayStats.best > 0 ? dayStats.best.toFixed(2)+'%' : '—';
  el = document.getElementById('day-earned');
  if (el) el.textContent = '$' + (dayStats.earned||0).toFixed(0);
  var avg = dayStats.spreads.length ? (dayStats.spreads.reduce(function(a,b){return a+b;},0)/dayStats.spreads.length).toFixed(2) : '—';
  el = document.getElementById('day-avg');
...
function playSound(type) {
  try {
    ...
    if (type === 'expire') {
      // Таймер истёк — три нисходящих тона
      ...
      // Фиксация прибыли — победный джингл вверх
      [523,659,784,1047].forEach(function(freq, i) {
        ...
        g.gain.linearRampToValueAtTime(0.12, ctx.currentTime + i*0.1 + 0.04);
      ...
      // Сделка зафиксирована через админку — два восходящих тона
