...
function openMobSettings(){
  ...
  if(suppLink&&currentUser?.id) suppLink.href='https://t.me/RavenEyeSupport?text='+encodeURIComponent('Привет! Мой ID: '+currentUser.id+'\nПроблема: ');
  // Theme toggle state
...
function setActiveTrade(trade){
  trade._tookAt = Date.now();
  trade._autoFixed = false;
  activeTimerTrade = trade;
  // Сохраняем в localStorage для восстановления после перезагрузки
  try{ localStorage.setItem('dex_active_trade', JSON.stringify({trade: trade, tookAt: trade._tookAt})); }catch(e){}
...
function clearActiveTrade(){
  ...
  try{ localStorage.removeItem('dex_active_trade'); }catch(e){}
  updatePcTimer();
...
function renderTimerScreen(){
  ...
  // Calculate end time: use createdAt timestamp or tookAt time
  // autoDate у авто-сделок — строка "2026-04-25", не timestamp
  // Используем tookAt (время когда взяли сделку) или createdAt
  var startedAt;
  if(t._tookAt){
    startedAt = t._tookAt; // время когда нажали "Взять сделку"
  } else if(t.createdAt && t.createdAt.seconds){
    startedAt = t.createdAt.seconds * 1000;
  } else {
    startedAt = Date.now(); // fallback — считаем от сейчас
  }
  // Сохраняем время взятия сделки
  if(!t._tookAt){ t._tookAt = startedAt; }

  const windowMs = (t.tmax||30) * 60 * 1000;
  const fixMs    = (t.tmin||5)  * 60 * 1000; // время наступления фиксации
  const endTime  = startedAt + windowMs;
  const fixTime  = startedAt + fixMs;
  ...
  function tick(){
    ...
    // Авто-фиксация: когда наступает время tmin — помечаем сделку как fixed
    if(activeTimerTrade && !activeTimerTrade._autoFixed && Date.now() >= fixTime){
      activeTimerTrade._autoFixed = true;
      const tradeId = activeTimerTrade._id;
      if(tradeId && typeof apiCall === 'function'){
        apiCall('fixTrade', {id: tradeId, fixed: true}).catch(function(){});
      }
    }
...
async function confirmFixProfit(){
  ...
  if(!navigator.onLine){
    ...
    if(typeof saveBalanceHistory==='function') saveBalanceHistory(currentUser.balance);
    if(typeof updateTopbarChart==='function') updateTopbarChart();
    return;
  }
  // Валидация: защита от накрутки баланса
  const currentBal = currentUser.balance||0;
  const newBalance = Math.round((currentBal + amount)*100)/100;
  // Максимум +$10000 за одну фиксацию
  if(amount > 10000){
    showToast('Сумма слишком большая. Максимум $10,000 за раз.','error');
    btn.disabled=false; btn.textContent='Подтвердить';
    return;
  }
  // Баланс не может быть отрицательным
  if(newBalance < 0){
    showToast('Баланс не может быть отрицательным','error');
    btn.disabled=false; btn.textContent='Подтвердить';
    return;
  }
  // Максимальный баланс $1,000,000
  if(newBalance > 1000000){
    showToast('Превышен максимальный баланс','error');
    btn.disabled=false; btn.textContent='Подтвердить';
    return;
  }
  try{
    // Пробуем через защищённый API
    var apiResult = await apiCall('fixProfit', {
      userId: currentUser.id,
      amount: amount,
      newBalance: newBalance
    });
    if (apiResult && !apiResult.ok) {
      showToast('Ошибка: ' + apiResult.error, 'error');
      btn.disabled=false; btn.textContent='Подтвердить';
      return;
    }
    if (!apiResult) {
      // Fallback — напрямую в Firebase (если API недоступен)
      console.log('[Fix] API unavailable, using direct Firebase write');
      await db.collection('users').doc(currentUser.id).update({
        balance: newBalance,
        updatedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
    } else if (apiResult.ok) {
      console.log('[Fix] API success, newBalance:', apiResult.newBalance || newBalance);
    }
    ...
    // Сохраняем историю баланса для графика
    if(typeof saveBalanceHistory==='function') saveBalanceHistory(newBalance);
    if(typeof updateTopbarChart==='function') updateTopbarChart();
...
const volMap={0:'mf-vol-0',3000000:'mf-vol-3m',5000000:'mf-vol-5m',10000000:'mf-vol-10m'};
...
(function(){
  ...
  document.addEventListener('touchend', function(e){
    try {
      ...
      e.preventDefault();
      var id = t.id;
      if(id==='profile-open-btn'){
        setTimeout(function(){ if(typeof openProfileModal==='function') openProfileModal(); }, 50);
      } else if(id==='theme-btn'){
        if(typeof applyTheme==='function') applyTheme(currentTheme==='dark'?'light':'dark');
      } else if(id==='alert-btn'){
        if(typeof window.toggleAlerts==='function') window.toggleAlerts();
      } else if(id==='tg-connect-btn'){
        if(typeof showTgRegModal==='function') showTgRegModal();
      } else if(id==='logout-btn'){
        if(window.doLogout) window.doLogout();
      }
    } catch(err) { console.log('topbar touch err:', err.message); }
  }, {passive:false});
...
function tickPcTimer() {
  ...
  var t = activeTimerTrade;
  var startedAt = t._tookAt || (t.createdAt && t.createdAt.seconds ? t.createdAt.seconds*1000 : Date.now());
  if(!t._tookAt){ t._tookAt = startedAt; }
  var fixMs     = (t.tmin||5)  * 60 * 1000;
  var windowMs  = (t.tmax||30) * 60 * 1000;
  var fixTime   = startedAt + fixMs;
  var endTime   = startedAt + windowMs;
  var remaining = Math.max(0, endTime - Date.now());
  var pct       = Math.max(0, Math.min(1, remaining / windowMs));

  // Авто-фиксация при достижении tmin
  if(!t._autoFixed && Date.now() >= fixTime){
    t._autoFixed = true;
    if(t._id && typeof apiCall === 'function'){
      apiCall('fixTrade', {id: t._id, fixed: true}).catch(function(){});
    }
  }
