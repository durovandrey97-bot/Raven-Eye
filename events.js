// ═══ events.js — вся обработка событий, навигация, таймер, фильтры ═══

import { CHAIN_META, NET_THRESHOLDS, PROFIT_HISTORY_KEY }        from './config.js';
import { trades, currentUser, db, chain, sortMode, sortDir,
         minSpread, minVol, hiddenNets, profitHistory, alerts,
         activeTimerTrade, timerInterval, knownIds, seenTradeIds,
         setChainFilter, setSortMode, setSortDir, setMinSpread,
         setMinVol, setFilterDex, setHiddenNets, setActiveTimerTrade,
         setTimerInterval, setAlertsOn, setProfitHistory,
         saveProfitEntry }                                        from './state.js';
import { doLogin, doLogout, tgDisconnect, tgSendToUser }          from './firebase.js';
import { render, fmtVol, fmtBal, isExpired, showToast,
         launchConfetti, openProfileModal, openDetailModal,
         updateMobAlertBadge, drawBalanceChart, saveBalanceHistory,
         updateConnectionStatus }                                 from './ui.js';
...
function mobNav(tab,btn){
  ...
  if(tab==='signals'){
    ...
    closeMobCalc(false);closeMobSettings(false);closeMobTimer(false);
    document.getElementById('profile-open-btn')?.click();
...
window._addBalanceManual = function(){
  ...
  ov.querySelector('#_bo').onclick = function(){
    ...
    if (!currentUser || !db) return;
    const { updateDoc, doc } = window._fbFns;
...
window._clearProfitHistory = function(){
  ...
  ov.onclick = e => { if (e.target === ov) ov.remove(); };
  ov.querySelector('#_co').onclick = function(){
    ov.remove();
    setProfitHistory([]);
    localStorage.removeItem(PROFIT_HISTORY_KEY);
